...
mod tests {
    ...
    async fn test_basic() {
        ...
        assert!(punish_verify(&signature, user, amount, proof_id, &nonce_manager).is_ok());
    }
    ...
    async fn test_bad_user() {
        ...
        assert!(punish_verify(&signature, bad_user, amount, proof_id, &nonce_manager).is_err());
    }
    ...
    async fn test_bad_amount() {
        ...
        let signature = punish_sign(&private_key, user.clone(), amount, proof_id, &nonce_manager)
            ...
            .expect("Should generate signature");
        let bad_amount = 200;
        assert!(punish_verify(&signature, user, bad_amount, proof_id, &nonce_manager).is_err());
    }
    ...
    async fn test_bad_proof_id() {
        ...
        assert!(punish_verify(&signature, user, amount, bad_proof_id, &nonce_manager).is_err());
    }
}
