...
mod tests {
    ...
    async fn test_basic() {
        ...
        assert!(vouch_verify(&signature, vouchee, &nonce_manager).is_ok());
    }
    ...
    async fn test_bad_user() {
        ...
        assert!(vouch_verify(&signature, bad_user, &nonce_manager).is_err());
    }
    ...
    async fn test_bad_nonce() {
        ...
        assert!(vouch_verify(&signature, vouchee, &nonce_manager).is_err());
    }
    ...
    async fn test_consumed_nonce() {
        ...
        let signature = vouch_sign(&private_key, vouchee.clone(), &nonce_manager)
            .await
            .expect("Should generate signature");
        assert!(vouch_verify(&signature, vouchee.clone(), &nonce_manager).is_ok());
